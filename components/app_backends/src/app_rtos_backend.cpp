...
QueueHandle_t sample_request_queue = nullptr;
QueueHandle_t sensor_sample_queue = nullptr;
...
TaskHandle_t fault_task_handle = nullptr;

portMUX_TYPE sample_timestamp_mux = portMUX_INITIALIZER_UNLOCKED;
std::uint32_t last_sample_timestamp_ms = 0;

#if defined(ENABLE_STRESS_MODE)
StressModeState stress_mode_state;
#endif

void record_sample_timestamp(std::uint32_t timestamp_ms) {
    portENTER_CRITICAL(&sample_timestamp_mux);
    last_sample_timestamp_ms = timestamp_ms;
    portEXIT_CRITICAL(&sample_timestamp_mux);
}

std::uint32_t get_last_sample_timestamp() {
    portENTER_CRITICAL(&sample_timestamp_mux);
...
void trace_pulse_begin(std::uint8_t pin) {
    firmware_hal::gpio_write_level(pin, true);
}

void trace_pulse_end(std::uint8_t pin) {
    firmware_hal::gpio_write_level(pin, false);
}
#else
void trace_pins_init() {}
void trace_pulse_begin(std::uint8_t) {}
void trace_pulse_end(std::uint8_t) {}
#endif
...
void fault_task(void* parameter) {
    (void)parameter;
    (void)watchdog_register_current_task();
